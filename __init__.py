...
class UnderratedAlbumsSkill(OVOSCommonPlaybackSkill):
    def __init__(self):
        super().__init__("UnderratedAlbums")
        self.skill_icon = self.default_bg = join(dirname(__file__), "ui", "underratedalbums_icon.jpg")
        self.supported_media = [MediaType.GENERIC,
                                MediaType.MUSIC]
        # load video catalog
        self.archive = YoutubeMonitor("UnderratedAlbums", logger=LOG, blacklisted_kwords=["subscribe"])
        self.n_mixes = 5
    ...
    @ocp_search()
    def ocp_underrated_albums(self, phrase, media_type):
